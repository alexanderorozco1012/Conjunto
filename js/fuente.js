...
function crearTabla(){
    if(tableStatement!=="" && tableStatement.length!==0){
        ...
        ventana+='<table border="0">';
        ventana+='<caption><label><strong>Tabla de Verdad</strong></label></caption>';
        ventana+='<thead>';
        ventana+='<tr>';
        for(var i=0; i<verdades.length; i++){
            if(verdades[i]){
                ventana+='<th><label>'+proposiciones[i].toUpperCase()+'</label></th>';
            }
        }
        if(tableStatement.length===1){
            ventana+='<th><label>'+proposicionesSimples[0].toUpperCase()+'</label></th>';
        }else{
            generarEncabezado();
        ...
        for(var idx=0; idx<rows; idx++){
            ...
            if(answers.length>0){
                for(var index=0; index<answers.length; index++){
                    ventana+='<td><label>'+ answers[index].toUpperCase() +'</label></td>';               
                }
        ...
        ventana+='</table>';
        var tipo = tipoTabla();
        ventana+='<div ALIGN="center"><label>Es una tabla de <strong style="color:#F06F3E">'+ tipo +'</strong></label></div>'; 
        ventana+='<div ALIGN="center">';
        ventana+='<input type="button" value="Cerrar" onclick="cerrarTabla()" />';
        ventana+='</div>';
        $('.modal').css('top', '50%');
         $('.modal').css('opacity', '1');
    
        $('.oscuro').css('display', 'block');
        $('.modal').html(ventana)
...
function Comprobador(){
    ...
    this.Proposicion = function(x){
          ...
          switch(x){
             ...
             case 't' : sw = true; break;
             case 'x' : sw = true; break;
          }  
	  return sw;
    };
}